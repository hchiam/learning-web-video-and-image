...
(function() {
  ...
  _.max = function(obj, iterator, context) {
    ...
    let result = {computed: -Infinity, value: -Infinity};
    each(obj, function(value, index, list) {
  ...
  const group = function(behavior) {
    return function(obj, value, context) {
      const result = {};
      const iterator = value == null ? _.identity : lookupIterator(value);
  ...
  _.zip = function() {
    const length = _.max(_.pluck(arguments, 'length').concat(0));
  ...
  _.bindAll = function(obj) {
    const funcs = slice.call(arguments, 1);
  ...
  _.times = function(n, iterator, context) {
    ...
    for (let i = 0; i < n; i++) accum[i] = iterator.call(context, i);
    return accum;
  };
...
if (typeof document!=='undefined'&&!('classList' in document.createElement('a'))) {
  (function(j) {
    if (!('HTMLElement' in j)&&!('Element' in j)) {
      ...
      const s=arguments; let r=0; const p=s.length; let q; let o=false; do {
        q=s[r]+''; if (g(this, q)===-1) {
          this.push(q); o=true;
        }
      } while (++r<p);if (o) {
      ...
      const t=arguments; let s=0; const p=t.length; let r; let o=false; do {
        r=t[s]+''; const q=g(this, r); if (q!==-1) {
      ...
      p+=''; const o=this.contains(p); const r=o?q!==true&&'remove':q!==false&&'add'; if (r) {
        this[r](p);
      } return !o;
    }; e.toString=function() {
      return this.join(' ');
...
(function() {
  ...
  window.CHARLIE = {};
  const CHARLIE = window.CHARLIE;


  /** **********************************************************************
       * Constants
       */
  const KEYFRAMES_RULE = window.CSSRule.KEYFRAMES_RULE ||
  ...
  /** **********************************************************************
       * Helper Functions
       */
  const scrapeAnimationData = function() {
    ...
    _.forEach(
        ...
        function(element) {
          ...
          //
          const names = element.dataset.animations.split(/\s*,\s*/);
          const times = element.dataset.times.split(/\s*,\s*/);

          // creates an array of arrays, each one called a 'tuple'
          // basically ties the time to the
          // animation name, so it looks like:
          // [["zippy", 1], ["fade", 2] ... ]
          const tuples = _.zip(names, times);
  ...
  /*
      ...
      */
  const findRules = function(matches) {
    const styleSheets = _.toArray(document.styleSheets);
    const rules = [];
  ...
  const roundTime = function(time) {
    // round a time to one tenth of a second
    // return time.toFixed(1);
    return Math.round(time * 10) / 10;
  };

  const animationName = (function() {
    let name = '';
  ...
  const prefixed = function(prop) {
    const props = _.map(PREFIXES, function(prefix) {
  ...
  const animationDuration = (function() {
    let durationName = '';
    const props = prefixed('animationDuration');
  ...
  const setDelay = function(animation, seconds) {
    var delay = -(seconds - animation.startsAt);
    var delay = delay < 0 ? delay : 0;
    const milliseconds = Math.floor(delay * 1000) + 'ms';
  ...
  const CSSAnimations = function(keyframes, cssRules) {
  ...
  CSSAnimations.create = function() {
    ...
    const keyframeRules = findRules(function(rule) {
    ...
    const animationStyleRules = findRules(function(rule) {
    ...
    const cssRules =
              _.object(
                  _.map(
                      animationStyleRules,
                      function(style) {
                        return [style.selectorText.substring(1), style];
                      }));
  ...
  const AnimationController = function(animations, bySeconds, timeModel, callbacks) {
  ...
  AnimationController.prototype = {
    ...
    startAnimations: function(time, videoTime) {
      ...
      const seconds = roundTime(videoTime);
      const me = this;
      ...
      if (me.bySeconds[seconds]) {
        const animations = me.bySeconds[seconds];
        const notRunning = _.filter(animations, function(animation) {
    ...
    executeCallbacks: (function() {
      ...
      return function(time, videoTime) {
        ...
        if (seconds > currentTime || seconds < currentTime) {
          currentTime = seconds;
          const callbacks = me.callbacks[seconds] || [];
    ...
    seek: (function() {
      const animationsToStart = function(me, seconds) {
        const toStart = [];

        for (let i = 0; i < me.timeModel.length; i++) {
          const animation = me.timeModel[i];
      ...
      return function(videoTime, playNow) {
        ...
        const me = this;
        const seconds = roundTime(videoTime);
        const toStart = animationsToStart(me, seconds);
    ...
    bind: (function() {
      const createAnimations = function(me, cssAnimations, startTimes, callbacks) {
        _.forEach(_.keys(startTimes),
            function(name) {
              const keyframe = cssAnimations.keyframes[name];
              const cssRule = cssAnimations.cssRules[name];
      ...
      const createTimeModel = function(me, animations) {
      ...
      return function(cssAnimations, startTimes) {
        ...
        const animations = _.flatten(_.values(me.animations));
  ...
  /** **********************************************************************
       * BigLoop
       */
  const BigLoop = function(controller) {
  ...
  CHARLIE.setup = function(video) {
    const cssAnimations = CSSAnimations.create();
    const animationData = scrapeAnimationData();
    const controller = new AnimationController();
    const loop = new BigLoop(controller);
